...
public class PersonController {
    ...
    public ResponseEntity<PersonDetailsDTO> getPersonByLogin(@PathVariable("username") String username,
                                                             @PathVariable("password") String password) {
        PersonDetailsDTO dto = personService.findPersonByLogin(username, password);
        return new ResponseEntity<>(dto, HttpStatus.OK);
    }
    ...
    @PutMapping(value = "/{id}")
    public ResponseEntity<UUID> updatePerson(@RequestBody PersonDetailsDTO personDTO, @PathVariable("id") UUID personId) {
        UUID personID = personService.update(personDTO, personId);
        return new ResponseEntity<>(personID, HttpStatus.OK);
    }

    @DeleteMapping(value = "/{id}")
    public ResponseEntity<UUID> deletePerson(@PathVariable UUID id) {
